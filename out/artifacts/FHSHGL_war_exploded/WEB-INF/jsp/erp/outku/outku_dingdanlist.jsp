...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="outku/dingdanList.do" method="post" name="Form" id="Form">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:20px;"><span class="label label-info arrowed-right arrowed-in">订单记录</span></td>
							</tr>
						</table>
						<!-- 检索  -->
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>
												<i style="cursor:pointer;" onclick="viewDingdan('${var.ORDER_NUMBERS}','${var.ZPRICE}','${var.CUSTOMER_NAME}','${var.OUTTIME}');" id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue">
												${var.ORDER_NUMBERS}
											</td>
											<td class='center'>${var.ZPRICE}</td>
											<td class='center'>${var.OUTDATE}</td>
											<td class='center'>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增订单";
			 diag.URL = '<%=basePath%>outku/goAddDingdan.do';
			 diag.Width = 1000;
			 diag.Height = 700;
		...
		//订单详情
		function viewDingdan(ORDER_NUMBERS,ZPRICE,CUSTOMER_NAME,OUTTIME){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="订单详情";
			 diag.URL = '<%=basePath%>outku/listAllByDingdan.do?ORDER_NUMBERS='+ORDER_NUMBERS+'&ZPRICE='+ZPRICE+'&CUSTOMER_NAME='+CUSTOMER_NAME+'&OUTTIME='+OUTTIME;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = false;			//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
